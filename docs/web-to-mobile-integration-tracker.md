...
| 2026-09-09 | Added automatic AI evaluation for STEM Engineer-created Monthly and Annual assessments. | Web `POST /assessment-results/store` and mobile `POST /api/student/assessment-sessions/{sessionId}/submit` now pass saved submissions through a shared Gemini evaluator when the approved assessment belongs to a STEM Engineer and category is Monthly/Annual. AI-generated papers use the stored JSON blueprint; uploaded PDF papers use protected PDF text extraction when available. Annual passes still create the existing pending certificate approval request. If AI/PDF extraction is unavailable, the result remains `Pending Review` for manual evaluation. | Flutter assessment submission now reads the API message so students can see whether evaluation completed instantly or is awaiting review. Existing result APIs already expose status, score, percentage, pass state, and feedback. |
| 2026-09-09 | Added STEM Engineer review/override for AI-evaluated Monthly and Annual assessment results. | Web assessment review now includes AI-evaluated rows by default, adds an `AI Evaluated` status filter, and lets STEM Engineers confirm or adjust AI marks, pass/fail, and feedback through the existing review form. Annual certificate eligibility is resynced after override so pending certificates are removed if the adjusted result no longer qualifies. | Mobile assessment review workflow now labels AI-scored rows as `AI Evaluated`, exposes the evaluation source, and changes the review action label to `Review AI score` while reusing the existing score/feedback/pass override API. |
